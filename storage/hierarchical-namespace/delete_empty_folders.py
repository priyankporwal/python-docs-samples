...
from google.cloud import storage  # Required for object-level operations
...
# 0. Object Deletion (Optional):
#    - Deletes all objects within the prefix in parallel batches of 100.
# 1. Folder Discovery:
...
# objects (immediate or nested) will fail to be deleted unless DELETE_OBJECTS_FIRST is True.
#
...
# Set to True to clear all files before folder deletion starts.
DELETE_OBJECTS_FIRST = True
...
def clear_all_objects_batched():
  ...
  logging.info(
      "Starting parallel object deletion for bucket '%s' with prefix '%s'.",
      BUCKET_NAME, FOLDER_PREFIX
  )
  
  # List all blobs. We exclude names ending in '/' as those are directory 
  # markers which are handled by the folder deletion logic later.
  ...
  # Split into 100-item chunks (GCS API limit for batching)
  chunk_size = 100
  chunks = [blob_names[i:i + chunk_size] for i in range(0, len(blob_names), chunk_size)]
  
  logging.info(f"Found {len(blob_names)} objects. Dispatching {len(chunks)} batch requests.")

  with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix="DeleteObjectWorker") as executor:
    results = list(executor.map(delete_batch_of_blobs, chunks))
  
  logging.info(f"Object deletion complete. Total objects removed: {sum(results)}")
...
if __name__ == "__main__":
  ...
  # Step 0: Clear objects if configured.
